      ...
      <div class="app-card app-card-account shadow-sm d-flex flex-column align-items-start">
        <div class="app-card-header p-3 border-bottom-0">
          <div class="row align-items-center gx-3">
            ...
            <!--//col-->
            <div class="col-auto">
              <h4 class="app-card-title">{{group.title}}</h4>
            </div>
            <!--//col-->
          </div>
          <!--//row-->
        ...
        <div class="app-card-body px-4 w-100">
          <div v-for="(field, index2) in (group.fields)" :key="index2">
            <div v-if="field.canShow" class="item border-bottom py-3">
              <div class="row justify-content-between align-items-center">
                <div class="col-auto">
                  <div class="item-label"><strong>{{field.label}}</strong></div>
                  <div class="item-data">{{model[field.name]}}</div>
                </div>
                <!--//col-->
              </div>
              <!--//row-->
...
export default defineComponent({
  ...
  data() {
    return {
      titlePage: this.viewmodel?.getTitleShow(),
